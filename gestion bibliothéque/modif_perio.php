...
// Modifier un périodique si une requête de modification est reçue
if (isset($_POST['update_id'])) {
    $update_id = $_POST['update_id'];
    $new_title = $_POST['new_title'];
    $new_year = $_POST['new_year'];
    $new_publisher = $_POST['new_publisher'];
    $new_issn = $_POST['new_issn'];
    $new_volume = $_POST['new_volume'];
    $new_price = $_POST['new_price'];

    // Requête pour mettre à jour les informations du périodique
    $stmt = $conn->prepare(
        "UPDATE Doc d JOIN Perio p ON d.id_doc = p.id_doc SET d.titre = ?, d.annee = ?, d.editeur = ?, p.issn = ?, p.vol = ?, p.prixp = ? WHERE d.id_doc = ?"
    );
    $stmt->bind_param("sissidi", $new_title, $new_year, $new_publisher, $new_issn, $new_volume, $new_price, $update_id);
    $stmt->execute();
    $stmt->close();

    echo "<div class='message success'>Le périodique a été modifié avec succès.</div>";
}
...
<div class="container">
        ...
        <tbody>
            ...
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><button onclick='showEditForm(" . $row['ID'] . ", \"" . addslashes($row['Titre']) . "\", " . $row['Annee'] . ", \"" . addslashes($row['Editeur']) . "\", \"" . addslashes($row['ISSN']) . "\", " . $row['Volume'] . ", " . $row['Prix'] . ")'>Modifier</button></td>";
                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='7'>Aucun périodique trouvé</td></tr>";
            }
            ?>
        </tbody>
    </table>
    ...
    <div id="editForm">
        ...
        <form method="POST">
            ...
            <div>
                <label for="new_issn">ISSN</label>
                <input type="text" id="new_issn" name="new_issn" required>
            </div>
            <div>
                <label for="new_volume">Volume</label>
                <input type="number" id="new_volume" name="new_volume" required>
            </div>
            <div>
...
<script>
    ...
    function showEditForm(id, title, year, publisher, issn, volume, price) {
        ...
        document.getElementById('new_issn').value = issn;
        document.getElementById('new_volume').value = volume;
