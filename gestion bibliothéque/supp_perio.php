...
// Supprimer un périodique si une requête de suppression est reçue
if (isset($_GET['delete_id'])) {
    $delete_id = $_GET['delete_id'];

    // Supprimer le périodique de la table perio
    $stmt = $conn->prepare("DELETE FROM perio WHERE id_doc = ?");
    ...
    echo "<div class='message' style='background-color: #d4edda; color: #155724;'>Le périodique a été supprimé avec succès.</div>";
}
        ...
        <tbody>
            ...
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><a href='?delete_id=" . $row['ID'] . "' class='delete-button' onclick='return confirm(\"Êtes-vous sûr de vouloir supprimer ce périodique ?\");'>Supprimer</a></td>";

                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='9'>Aucun périodique trouvé</td></tr>";
            }
            ?>
        </tbody>
    </table>
...
function filterTable() {
    ...
    const rows = document.querySelectorAll('#periodicalTable tbody tr');
